<template>
    <div class="container">
        <div class="container-top">
            <div>
                <span style="color:var(--color-text-2);">设备</span> <a-select :style="{ width: '320px' }"></a-select>
                <a-date-picker v-model="value" style="width: 320px;margin-left: 100px;" />
            </div>
            <div>
                <a-popover position="lt" trigger="click" title="指标说明">
                    <div style="color:var(--color-text-2);"><icon-info-circle-fill /><span>指标说明</span></div>
                    <template #content>
                        <a-divider></a-divider>
                        <p>如果有电压/电流三相不平衡度变量，直接使用该变量的数据；</p>
                        <p>如果没有电压/电流三相不平衡度变量，电压/电流三相不平衡度=（相最大-相最小）/ 相最大</p>
                    </template>
                </a-popover>
            </div>
        </div>
...
onMounted(() => {
    ...
    const myChartTwo = echarts.init(document.getElementById('myChartTwo'), null, {
        height: 400
    })

    const myChartSecond = echarts.init(document.getElementById('myChartSecond'), null, {
        height: 400
    })

    const myChartSecondTwo = echarts.init(document.getElementById('myChartTwoSecond'), null, {
        height: 400
    })
