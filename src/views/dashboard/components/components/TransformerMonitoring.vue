<template>
  <div class="container">
    <div class="header">
      ...
      <a-select :style="{ width: '320px' }" :default-value="dataList.title" placeholder="Please select ..." allow-search>
        ...
        <a-popover position="br" style="width: 600px" trigger="click">
          ...
          <template #content>
            ...
            <a-tabs default-active-key="1">
              <a-tab-pane key="1" title="运行状态说明" class="header-right-tab1">
                <a-table :columns="columns" :data="data" :body-cell-style="{ color: 'red' }" :pagination="false" />
              </a-tab-pane>
              <a-tab-pane key="2" title="配置说明" class="header-right-tab2">
                <div>
        ...
        <a-col :span="8">
          <div style="
            ...
            <div style="margin: 0 20px">
              <a-image style="width: 100%.padding 10px:;" src="http://118.190.51.135/img/dark/transformer/dry-type.png" />
            </div>
          </div>
          ...
          <div style="margin-top: 20px">
            <a-table :columns="columnsData" :data="dataLists" :bordered="{ cell: true }" :pagination="false"
              :stripe="true" />
          </div>
        </a-col>
        <a-col :span="16">
          <a-row :gutter="10">
            <a-col :xxl="{ span: '24-5' }" v-for="(itemList, itemListIndex) in rightData" :key="itemListIndex">
              <a-card style="font-size: 14px">
                {{ itemList.title }}
                <div v-for="(rightItem, rightItemIndex) in itemList.children" :key="rightItemIndex" style="
                    height: 36px;
                    background-color: #33404c;
                    margin: 4px 0;
                    padding: 10px 6px;
          ...
          <a-tabs default-active-key="1" @click="Clicks" @tab-click="handleClick">
            <a-tab-pane v-for="(tabsDate, index) in tabsList" :key="index + 1" :title="tabsDate.title">
            </a-tab-pane>
          </a-tabs>
          <div>
          ...
          <a-table :columns="rightBottomColumns" :data="rightBottomData" :bordered="{ cell: true }" :pagination="false"
...
import { ref, watch, reactive, onMounted, defineEmits } from "vue";
...
onMounted(() => {
  setTimeout(() => {
    myChart = echarts.init(document.getElementById("eCharts"), null, {
      height: 366,
    });
    const options = {
      tooltip: {
