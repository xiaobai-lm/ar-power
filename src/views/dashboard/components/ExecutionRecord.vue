<template>
    <div class="container">
        <a-radio-group v-model="checkValue" @change="handleChange">
            <a-radio value="1">手动</a-radio>
            <a-radio value="2">自动</a-radio>
        </a-radio-group>
        <div v-if="checkValue === '1'">
            ...
            <a-table :columns="columns" :data="dataHistoryTwo" style="margin-top: 40px;" :bordered="{ cell: true }" />
        </div>
        <div v-else>
            ...
            <span class="historySpan">执行时间：</span> <a-range-picker v-model="rangeValue" style="width: 300px;" />
            <a-button type="primary">刷新</a-button>
            <a-table :columns="columns" :data="dataHistory" style="margin-top: 40px;" :bordered="{ cell: true }" />
        </div>
    </div>
</template>
