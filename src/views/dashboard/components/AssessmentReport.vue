<template>
    <div class="container">
        <a-tabs type="card-gutter" class="tabsList" v-model:active-key="activeKey">
            <a-tab-pane key="1" title="实时评分">
                <div
                    style="display:flex;justify-content:space-between ; height: 40px;line-height: 40px;color: var(--color-text-1);;">
                    <div>数据刷新时间：{{ date }}
                    ...
                    <a-drawer :width="540" :visible="drawVisible" @ok="drawHandleOk" @cancel="drawHandleCancel"
                        unmountOnClose :footer="false">
                        <template #title>
                            指标说明
                        </template>
                        <div class="draw-div">
                            <a-tabs>
                                <a-tab-pane key="1" title="配置内容">
                                    <div v-for="(item, index) in drawList" :key="index">
                                        <div
                                            style="--color-text-1: rgba(255, 255, 255, 0.9);font-size: 18px;margin: 10px 0;">
                                            {{ item.title }}</div>
                                        <div style="font-size: 15px;margin: 10px 0;">{{ item.content }}</div>
                                        <div style="white-space: pre-line;color: #7e93a6;font-size: 14px;margin: 10px 0;">
                                            {{ item.data }}
                                        </div>
                                        <div style="color: #7e93a6;font-size: 12px;margin: 10px 0;">
                                            {{ item.value }}
                                        </div>
                                        <a-divider></a-divider>
                                    </div>
                ...
                <a-row :gutter="20">
                    <a-col :span="6" style="position:relative">
                        <a-card title="评分" style="margin: 0 auto">
                            ...
                            <div :style="radarStyle" id="radarChartValue">
                                {{ achievement }}
                            </div>
                        </a-card>
                    ...
                    <a-col :span="18">
                        <a-table :columns="columns" :data="data" :scroll="scroll" :bordered="{ cell: true }"
                        @cell-mouse-enter="cellMouseEnter" :cell-mouse-leave="cellMouseLeave" @cell-click="cellClick" @row-click="RowClick" >
                        </a-table>
            ...
            <a-tab-pane key="2" title="单回路监测">
                <div style="display: flex;justify-content:space-between;height: 40px;line-height: 40px;margin: 0 0 10px 0;">
                        ...
                        <a-card title="三相不平衡度">
                            <div>
                                <a-row :gutter="10">
                                    ...
                                    <a-col :span="12">
                                        <div
                                            style="margin-top:40px;text-align: center ;width: 80%;height: 60px;padding: 14px 0 0 0;background: linear-gradient(-20deg, rgb(81, 198, 255), rgb(21, 72, 248))">
                                            <div>电压不平衡度</div>
                                            {{ averageValue }}%
                                        </div>
                                ...
                                <a-card title="频率">
                                    <div style="text-align: center;">
                                        <div style="margin: 20px;">
                                            ...
                                            <a-space
                                                style="width: 70px;height: 20px;text-align: center;border-radius: 10px;font-size: 16px">50.05Hz</a-space>
                                        </div>
                                        <div>
                                            <span style="font-size: 18px">频率偏差 </span>
                                            <a-space
                                                style="width: 80px;height: 30px;background-color: #1dc593;font-size: 16px;border-radius: 15px;">0.04%</a-space>
                                        </div>
                                    </div>
                                    <div class="frequency-footer">
                                        <a-divider class="frequency-footer-divider" type="dotted" direction="vertical" />
                                        <a-divider class="frequency-footer-divider2" type="dotted" direction="vertical" />
                                        <a-divider class="frequency-footer-divider3" type="dotted" direction="vertical" />
                                        <a-divider class="frequency-footer-divider4" type="dotted" direction="vertical" />
                                        <a-divider class="frequency-footer-divider5" type="dotted" direction="vertical" />
                                        <div style="position: absolute;left: 12%;top: 130%;">1</div>
                                        <div style="position: absolute;left: 32%;top: 130%;">1</div>
                                        <div style="position: absolute;left: 50%;top: 130%;">1</div>
                                        <div style="position: absolute;left: 72%;top: 130%;">1</div>
                                        <div style="position: absolute;left: 92%;top: 130%;">1</div>
                                ...
                                <a-card title="有功功率和无功功率">
                                    <div style="margin: 35px 20px;">
                                        ...
                                        <a-popover>
                                            ...
                                            <a-progress :percent="HavePower"
                                                :style="{ width: '50%', height: '10px', marginTop: '30px' }" />
                                        </a-popover>
                                        {{ kw }} kw
                                        <br>
                                        无功功率：
                                        <a-popover>
                                            ...
                                            <a-progress :percent="NoPower" :style="{ width: '50%', margin: '52px 0' }" />
                                        </a-popover>
                                        {{ kvar }} kVar
                                    </div>
                                </a-card>
...
const cellMouseEnter = (record: any,column: any, event: any) =>{
    console.log(record);
    console.log("aaaaaaaa");
}

const cellMouseLeave = (record: any,column: any, event: any) =>{
    console.log(record);

    console.log("aaaaaaaammmmmmmmmmmm");
}
...
if (integer > 80) {
    ...
    radarStyle.value = "background: linear-gradient(-20deg, rgb(81, 198, 255), rgb(21, 72, 248))"
} else if (integer > 60) {
    achievement.value = "合格"
    radarStyle.value = "background: linear-gradient(-20deg, rgb(250, 227, 159), rgb(236, 119, 55))"
} else if (integer <= 60 && integer >= 0) {
    achievement.value = "不合格"
    radarStyle.value = "background: linear-gradient(-20deg, rgb(255, 142, 167), rgb(255, 56, 66))"
}
onMounted(() => {
    const radarChart = echarts.init(document.getElementById("radarChart"), null, {
        height: 400
    })
    ...
    const seriesChart = echarts.init(document.getElementById("seriesChart"), null, {
        height: 400,
    })
    ...
    const seriesChartOptions = {
        ...
        color: [{
            ...
            y2: 1,
            colorStops: [{
                offset: 0, color: 'rgb(81, 198, 255)' // 0% 处的颜色
            }, {
                offset: 1, color: 'rgb(21, 72, 248)' // 100% 处的颜色
            }],
        ...
        },
        {
            ...
            y2: 1,
            colorStops: [{
                offset: 0, color: 'rgb(250, 227, 159)' // 0% 处的颜色
            }, {
                offset: 1, color: 'rgb(236, 119, 55)' // 100% 处的颜色
            }],
            global: false // 缺省为 false
        },
        {
            ...
            y2: 1,
            colorStops: [{
                offset: 0, color: 'rgb(255, 142, 167)' // 0% 处的颜色
            }, {
                offset: 1, color: 'rgb(255, 56, 66)' // 100% 处的颜色
            }],
            global: false // 缺省为 false
        }
        ],
    ...
    const modalChart = echarts.init(document.getElementById("modalChart"), null, {
        height: 400
    })
    ...
    const threePhase = echarts.init(document.getElementById("threePhase"), null, {
        height: 300,
    })
    ...
    const threePhaseTwo = echarts.init(document.getElementById("threePhaseTwo"), null, {
        height: 300,
    })
    const gaugeData = [
        {
    ...
    const voltageChart = echarts.init(document.getElementById("voltageChart"), null, {
        height: 200
    })
    ...
    const contentTwo = echarts.init(document.getElementById("contentTwo"), null, {
        height: 260
    })
    ...
    const contentTwoA = echarts.init(document.getElementById("contentTwoA"), null, {
        height: 260
    })
